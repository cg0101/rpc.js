...
var idGenerator = function (a) {
    return a
        ...
        : ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, idGenerator);
};
...
function RPC(wrapper, logger) {
    if (!(this instanceof RPC)) {
        return new RPC(wrapper, logger);
    }
    ...
    var commandsCallback = function (cmd) {
        ...
        if (cmd.command === resultCmd) {
            ...
            for (var p in cmd.data) {
                ...
                remoteObj[p] = getRemoteCallFunction(
                    p,
                    self.callbacks,
                    connection
                );
...
RPC.prototype.getServer = function () {
    var self = this;
    //创建 server
        ...
        var commandsCallback = function (cmd) {
            if (cmd.command === descrCmd) {
                ...
                try {
                    ...
                    var resultCommand = command(errorCmd, {
                        id: cmd.data.id,
                        err: err,
                    });
...
function getOnDataFn(commandsCallback, lengthObj) {
    ...
    return function (data) {
        ...
        if (lengthObj.bufferBytes && lengthObj.bufferBytes.length > 0) {
            var tmpBuff = new Buffer(
                lengthObj.bufferBytes.length + data.length
            );
